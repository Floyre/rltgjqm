...
구체적인 명령어는 제공하지 말고, 해결 방향성만 알려주세요.`;
      ...
      const response = await aiService.getAISuggestion(prompt);
  ...
  async getUserSolutionChoice(command, errorMessage, solutionOptions) {
    try {
      ...
      // 선택된 해결법의 구체적 명령어 요청
      await this.getSpecificSolution(command, errorMessage, options[selectedIndex]);
