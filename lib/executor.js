  ...
  async getUserSolutionChoice(command, errorMessage, solutionOptions) {
    try {
      ...
      const choices = options.map((option, index) => ({
        name: `${index + 1}. ${option.title}`,
        value: index
      }));
      
      // 직접 입력 옵션 추가
      const directInputIndex = options.length;
      choices.push({ name: `${directInputIndex + 1}. 💭 직접 해결 방법 입력`, value: directInputIndex });
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        if (titleMatch) {
          ...
          // 일반 텍스트인 경우 처음 50자를 제목으로 사용
          const title = fullText.length > 50 ? fullText.substring(0, 47) + '...' : fullText;
          options.push({
            title: title,
